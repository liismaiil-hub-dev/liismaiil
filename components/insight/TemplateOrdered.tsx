...
useEffect(() => {
       dispatch(setInsightTemplateAyahsSelected({ayahs:JSON.parse(insightTemplate.ayahs[0])}))
     }, [insightTemplate]);
    ...
    return (<div className={ `flex  border-1 border-blue-400/20 rounded-md flex-col w-full justify-start p-2  space-y-2 items-stretch `} >
        ...
        Template from ayah : &nbsp; [ {insightTemplateAyahsSelected[0].number} ] &nbsp; to : &nbsp;[ {insightTemplateAyahsSelected[insightTemplateAyahsSelected.length -1 ].number} ] &nbsp;
