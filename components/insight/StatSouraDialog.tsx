...
console.log({d, min:d.min, pos: _xScale(d.min) });
      return  ayVisualisation===AY_VISUALISATION.MIN || ayVisualisation===AY_VISUALISATION.MINMAX ?`${d.ayMin}:${d.min}`: ''
   })
.attr('x', (d, i ) => {
    console.log({d, min:d.min,ayLength: d.ayMax.split(' ').length });
    console.log({d, min:d.min, pos: boundedWidth/ d.ayMax.split(' ').length});
    return   _xScale(d.min ) -d.ayMin.length
  }    )
.attr('y', function(d, i){
    return  _yScale(i) 
 })
...
return   ayVisualisation===AY_VISUALISATION.MAX || ayVisualisation===AY_VISUALISATION.MINMAX ?`${d.ayMax}:${d.max}`: ''
...
//     return `${d.max}-${d.ayMax}`
  ...
  .style('font-size',`${ayVisualisation===AY_VISUALISATION.MAX ||ayVisualisation===AY_VISUALISATION.MINMAX ? '15px': '9px'}` )
      .style('font-weight','light') 
      .attr('transform',(d)  =>`translate(${  margin.right + (d.ayMax.length) },0)`)    
...
const _xAxisTop = axisBottom(xScaleAxis).tickSizeOuter(12)
  ...
  const svgRef = useRef();
     ...
     if(typeof insightTemplate !== 'undefined'&& insightTemplate && insightTemplate.souraNb>0 && insightTemplate.souraNb <115){
      ...
      async function getStat() {
        ...
        const [ayVisualisation, setMinBehavior] = useState(AY_VISUALISATION.MINMAX);
