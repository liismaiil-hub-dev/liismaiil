...
const SprintBoard = ({ sprints }: { sprints: StagesStagePrismaType[] }) => {
  ...
  useEffect(() => {
    if (typeof sprints !== 'undefined' && typeof sprints[0] != 'undefined' && sprints?.length > 0) {
      console.log({ sprints });
      const _sprints = sprints.map((st) => ({
      ...
      dispatch(setStageGridsContext({ sprints: _sprints }))
