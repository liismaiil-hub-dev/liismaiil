  ...
  return (
      <ScrollShadow >
        {typeof catStages !== 'undefined' && catStages && catStages.length > 0 &&
          ...
          <div   className="flex items-center justify-start gap-2 p-2 w-full flex-wrap ">
            {catStages?.map((grid: StagePrismaType, index: number) => {
//console.log({ grid });
              return( <div key={`${grid.stageId}_${index}`} className="flex items-center justify-center rounded-md  ">
