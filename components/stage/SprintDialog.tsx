...
import { addGuestToStage,  getLocalStagesByNbAndGrid,  getStageForSprint } from '@/actions/stage';

export default function SprintDialog({isOpen,  onOpen, onClose}:{isOpen:boolean,  onOpen:()=> void,  onClose:()=> void}) {
    const dispatch = useDispatch()
    const {sprintContext} = useSelector((state: RootStateType) => state.stage)
...
useEffect(() => {
 ...
 //getAllStagesByNbAndGrid({grid:stageSprintSelected.grid, souraNb:stageGridSelected.souraNb!})
 console.log(({grid:stageGridSelected.grid, souraNb:stageGridSelected.souraNb!}));
...
async function getAllStagesByNbAndGrid({souraNb, grid}:{souraNb:number, grid:number}) {
    const { success , stages} = await getLocalStagesByNbAndGrid({souraNb,grid})
  if(success && stages ){
    console.log({stages: JSON.parse(stages)});
