'use client'
import SprintModel from 'app/api/sprint/Sprint.model'
import { AyahTabletType, GridInput, GridType, SprintType, StageType } from 'app/api/sprint/sprint.types'
...
import { APP_ENV } from '@/api/graphql/host/host.types'
...
export default function Space() {
  ...
  /* const sprintsSerialized = await getSprints()
    console.log(sprintsSerialized)
    const titlesFromJson = sprintsSerialized.map(ttle => ttle?.split('.').shift())
    */
  useEffect(() => {

    (async () => await getTitles())()
  }, []);

  dispatch(setSprintsTitles({ sprints: titles }))
